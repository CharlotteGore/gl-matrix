...
import { Vec4, Mat4, Quat } from "./types";
 ...
 * @returns {vec4} out
 */
export const scale = (out: Vec4, a: Readonly<Vec4>, scale: number): Vec4 => {
  out[0] = a[0] * scale;
  out[1] = a[1] * scale;
  out[2] = a[2] * scale;
  out[3] = a[3] * scale;
 ...
 * @returns {vec4} out
 */
export const random = (out: Vec4, scale: number): Vec4 => {
  scale = scale || 1.0;
...
export const forEach = <T>(a: Float32Array, stride: number = 4, offset: number = 0, count: number = 0, fn: (a: Vec4, b: Vec4, arg: T) => void, arg: T): void => {
