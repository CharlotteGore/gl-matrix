...
import { Vec2, Mat4, Mat3, Mat2, Quat, Mat2d, Vec3 } from "./types";
 ...
 * @returns {vec2} out
 */
export const scale = (out: Vec2, a: Readonly<Vec2>, scale: number): Vec2 => {
  out[0] = a[0] * scale;
  out[1] = a[1] * scale;
  return out;
}
...
export const forEach = <T>(a: Float32Array, stride: number = 2, offset: number = 0, count: number = 0, fn: (a: Vec2, b: Vec2, arg: T) => void, arg: T): void => {
  let l: number;
